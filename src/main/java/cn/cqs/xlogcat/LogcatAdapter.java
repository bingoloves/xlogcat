...
final class LogcatAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = LayoutInflater.from(parent.getContext()).inflate(R.layout.logcat_item, parent, false);
    ...
    public LogcatInfo[] getDataArray() {
        synchronized (LogcatAdapter.class) {
            return mDataSet.toArray(new LogcatInfo[mDataSet.size()]);
        }
    }
    /**
     * 添加单条数据
